...
//let data = File.ReadAllLines @"\\psf\Home\Desktop\Digits\trainingsample.csv" 
...
// [ YOUR CODE GOES HERE! ]
//let imagesWithIntegers = Array.map (fun x -> int x) images 
...
type ImageData = { Number:int; Pixels:int[] }

let extractImageData data = 

    let splitData = data |> Array.map (fun (s:string) -> s.Split(',')) 
    let images = splitData.[ 1 .. ]
    let imagesWithIntegers = images |> Array.map(fun x -> Array.map (fun element -> int element) x)
    let imageDatas = imagesWithIntegers |> Array.map(fun x -> { Number = x.[0]; Pixels = x.[ 1 .. ] })
    imageDatas

let imageDatas = extractImageData data
...
let image_distance (image1 : int[]) (image2 : int[]) =
    let sum = Array.map2 (fun pixel1 pixel2 -> pown (pixel1 - pixel2) 2) image1 image2 |> Array.sum
    sqrt (float sum)
    
let example_different = image_distance (imageDatas.[1].Pixels) (imageDatas.[2].Pixels)
let example_thesame = image_distance imageDatas.[1].Pixels imageDatas.[1].Pixels
...
// [ YOUR CODE GOES HERE! ]
    imageDatas |> Array.minBy (fun x -> image_distance x.Pixels unknown) |> fun x -> x.Number
...
let validation_data = File.ReadAllLines @"\\psf\Home\Desktop\Digits\validationsample.csv" 

let validationDatas = extractImageData validation_data

let classificationResults = validationDatas |> Array.map (fun x -> classify x.Pixels)
let correctlyClassified = Array.map2(fun original classification -> if original.Number = classification then 1 else 0) validationDatas classificationResults |> Array.sum

let percentage = Convert.ToDouble(correctlyClassified) / 500.0 * 100.0
